...
class L1TCaloParamsStage1OnlineProdHI : public L1ConfigOnlineProdBaseExt<L1TCaloParamsRcd,l1t::CaloParams> {
private:
...
public:
  ...
  L1TCaloParamsStage1OnlineProdHI(const edm::ParameterSet&);
  ~L1TCaloParamsStage1OnlineProdHI(void){}
};

L1TCaloParamsStage1OnlineProdHI::L1TCaloParamsStage1OnlineProdHI(const edm::ParameterSet& iConfig) :
...
void L1TCaloParamsStage1OnlineProdHI::hexStringToVector(const std::string& s, std::vector<uint32_t>& v)
{
...
boost::shared_ptr<l1t::CaloParams> L1TCaloParamsStage1OnlineProdHI::newObject(const std::string& objectKey) {
  // Note: Key should be MP7_PP_CONF_KEY
  using namespace edm::es;

  std::string stage1CaloSchema = "CMS_S1CALOL2" ;
  edm::LogInfo( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Producing l1t::CaloParams with key =" << objectKey;
  ...
  if (objectKey.empty()) {
    edm::LogInfo( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Key is empty, returning empty CaloParams";
    return caloParams;
  }
  ...
  if( mp7_hi_conf.queryFailed() || mp7_hi_conf.numberRows() != 1 ){
    edm::LogError( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Cannot get MP7_HI_CONF" ;
  ...
  if (!hi_algo_regs_key.empty()) {
    ...
    if( hi_algo_regs.queryFailed() || hi_algo_regs.numberRows() != 1 ){
      edm::LogError( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Cannot get HI_ALGO_REGS" ;
      return boost::shared_ptr< l1t::CaloParams >() ;
    }
  ...
  if (!hf_energies_thr_lut_key.empty()) {
    ...
    if( hfThrLUT.queryFailed() || hfThrLUT.numberRows() != 1){
      edm::LogError( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Cannot get PILEUP_SUB_LUT" ;
      return boost::shared_ptr< l1t::CaloParams >() ;
    }
    ...
    L1TCaloParamsStage1OnlineProdHI::hexStringToVector(hexData, rawData);
  ...
  if (!centrality_lut_key.empty() && !ext_centrality_lut_key.empty()) {
    ...
    if( centralityLUT.queryFailed() || centralityLUT.numberRows() != 1){
      edm::LogError( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Cannot get CENTRALITY" ;
      return boost::shared_ptr< l1t::CaloParams >() ;
    }
    ...
    if( extCentLUT.queryFailed() || extCentLUT.numberRows() != 1){
      edm::LogError( "L1-O2O: L1TCaloParamsStage1OnlineProdHI" ) << "Cannot get EXT_CENTRALITY_LUT" ;
      return boost::shared_ptr< l1t::CaloParams >() ;
    }
    ...
    L1TCaloParamsStage1OnlineProdHI::hexStringToVector(hexData2, rawData2);
...
DEFINE_FWK_EVENTSETUP_MODULE(L1TCaloParamsStage1OnlineProdHI);
