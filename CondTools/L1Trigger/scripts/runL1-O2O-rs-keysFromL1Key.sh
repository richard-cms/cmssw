...
then
    ...
    echo "Writing to the online database"
    # DB_OPTIONS="outputDBConnect=oracle://cms_orcoff_prep/CMS_CONDITIONS outputDBAuth=/nfshome0/l1emulator/run2/o2o/prod" 
    # XXX COMMISSIONING XXX
    DB_OPTIONS="outputDBConnect=oracle://cms_orcon_prod/CMS_CONDITIONS outputDBAuth=/nfshome0/l1emulator/run2/o2o/prod"
fi

cmsRun $CMSSW_BASE/src/CondTools/L1Trigger/test/L1ConfigWriteRSOnline_cfg.py runNumber=${runnum} ${DB_OPTIONS} ${overwrite} ${forceUpdate} logTransactions=0 $OBJKEYS keysFromDB=0 print
o2ocode=$?
