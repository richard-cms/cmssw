...
{
  TString filenames[3] = {
    "Pyquen_EmEnrichedDijet_PtHat30_PartonPt0_ParticlePt0_eta30_TuneZ2_Unquenched_2760GeV_numEvent1000.root",
    "Pyquen_EmEnrichedDijet_PtHat80_PartonPt0_ParticlePt0_eta30_TuneZ2_Unquenched_2760GeV_numEvent1000.root",
    "Pyquen_EmEnrichedDijet_PtHat170_PartonPt0_ParticlePt0_eta30_TuneZ2_Unquenched_2760GeV_numEvent1000.root"
  };
  ...
  for(int i = 0; i < 3; ++i)
  {
    for(int j = 0; j < 3; ++j)
    {
      hists[i][j] = new TH1D(labels[i*3+j],";((photon || pi0 || eta) && status == 1) momId;Count",nBins,-maxEt,maxEt);
    }
	...
	if (gens[i]->et[photon] > maxet)
	{
      ...
      if(index > -1)
      {
	hists[i][0]->Fill(gens[i]->momId[index]);
